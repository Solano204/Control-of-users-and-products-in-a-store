...
public class PrintProviders extends Conexion {
    ...
    private final ArrayList<Provider> listCus;
    private DefaultTableModel modelTable;

    public PrintProviders(SystemMain system) {
        this.con = establihsConnection();
        ps = null;
        res = null;
        this.system = system;
        listCus = new ArrayList<>();
        modelTable = (DefaultTableModel) system.tbProviders.getModel();
    ...
    private void printCus() {
        try {
            final String querySql = "{CALL Select_Provider}"; // I call the PA
            ps = con.prepareCall(querySql);
            res = ps.executeQuery();
            while (res.next()) { // Fill the ArrayList
                Provider cus = new Provider(res.getString("DniProvider"), res.getString("NameProvider"), res.getString("TelephoneProvider"),
                        res.getString("CountryProvider"), res.getString("RegistrationDate"));
                listCus.add(cus);
            }
    ...
    public void fillOutTable() {
        ...
        for (int a = 0; a < tamList; a++) { // I fill the table
            ...
            arrayCust[3] = (String) listCus.get(a).getCountryOrigin();
            arrayCust[4] = (String) listCus.get(a).getRegistrationDate();
            modelTable.addRow(arrayCust);
        }
        system.tbProviders.setModel(modelTable); // I add the items in the model Table
    }
